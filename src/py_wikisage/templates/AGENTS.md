...
Subfolders under `raw/` (e.g. `papers/`, `articles/`, `repos/`, `experiments/`, `assets/`) categorize sources. Drop new files there, then run compile or `py-wikisage ingest <path>`.
...
`config.yaml` in the project root: `llm.provider`, `llm.model`, `llm.api_key` or `llm.api_key_env`.
